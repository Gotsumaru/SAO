...
func update_procedural_animation(delta: float) -> void:
    ...
    if is_moving and player.is_on_floor():
        ...
        _set_bone_rotation("UpperArm.L", Vector3(deg_to_rad(20) * swing, 0, 0))
        _set_bone_rotation("LowerArm.L", Vector3(deg_to_rad(10) * swing, 0, 0))
        _set_bone_rotation("UpperArm.R", Vector3(deg_to_rad(-20) * swing, 0, 0))
        _set_bone_rotation("LowerArm.R", Vector3(deg_to_rad(-10) * swing, 0, 0))
        _set_bone_rotation("UpperLeg.L", Vector3(deg_to_rad(-30) * swing, 0, 0))
        _set_bone_rotation("LowerLeg.L", Vector3(deg_to_rad(20) * swing, 0, 0))
        _set_bone_rotation("UpperLeg.R", Vector3(deg_to_rad(30) * swing, 0, 0))
        _set_bone_rotation("LowerLeg.R", Vector3(deg_to_rad(-20) * swing, 0, 0))
    else:
...
func _update_foot(name: String):
    ...
    var result := space.intersect_ray(PhysicsRayQueryParameters3D.create(from, to, 0x7FFFFFFF, [player.get_rid()]))
    if result:
